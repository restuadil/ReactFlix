    ...
    return (
        <div className="mx-10 py-3 relative border-b border-black">
            ...
            bg-gray-700 active:scale-125 active:text-white`}
                ...
                <button
                    className={`absolute right-7 top-1/2 transform -translate-y-1/2 bg-opacity-30 p-2 rounded-full py-5 ${startIndex === credit.length - 5 ? 'hidden' : ""}
                        bg-gray-700 active:scale-125 active:text-white`}
                    onClick={nextSlide}
                >
            ...
            <div className="flex flex-row flex-wrap justify-center">
                {credit && credit.slice(startIndex, startIndex + 6).map((item) => {
                    return (
                        <motion.div
                            key={nanoid()}
